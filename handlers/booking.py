...
from db.requests import get_lessons, get_free_times, get_or_create_user, get_lesson_by_id, create_record, \
    get_user_records, delete_record
...
async def choose_time(callback:CallbackQuery,state:FSMContext,session:AsyncSession):
    ...
    user=await get_or_create_user(session=session,tg_id=callback.from_user.id,user_name=callback.from_user.username or callback.from_user.full_name)
    lesson=await get_lesson_by_id(session,lesson_id)
    await create_record(session=session,user_id=user.id,lesson_id=lesson_id,selected_date=selected_data,selected_time=selected_time)
    await callback.answer()
...
async def my_records(callback: CallbackQuery, session: AsyncSession):
    records = await get_user_records(session, callback.from_user.id)
    if not records:
    ...
    for record in records:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Отменить запись",callback_data=f"delete_record_{record.id}")]])

        text = (
            f"Предмет: {record.lesson.name}\n"
...
async def delete_user_record(callback: CallbackQuery, session: AsyncSession):

    record_id = int(callback.data.split("_")[-1])

    success = await delete_record(session,record_id,callback.from_user.id)
    if success:
